...
class OrthoSlicer3D:
    ...
    def __init__(self, data, affine=None, axes=None, title=None, cmap="gray"):
        """
        Parameters
        ----------
        data : array-like
        ...
        for ii, xax, yax, ratio, label in zip([0, 1, 2], [1, 0, 0], [2, 2, 1],
            ...
            im = self._axes[ii].imshow(
                ...
                cmap=self._cmap, interpolation='nearest', origin='lower')
            self._ims.append(im)
        ...
        if self.n_volumes > 1 and len(self._axes) > 3:
            ...
            yl = [lim + s * np.diff(lims)[0]
                  for lim, s in zip(yl, [-1.01, 1.01])]
